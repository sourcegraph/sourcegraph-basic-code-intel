...
import { makeEnvelope, range1, textDocument, position } from './util.test'
...
describe('hoverForPosition', () => {
    it('should correctly parse result', async () => {
        ...
        assert.deepStrictEqual(await hoverForPosition(textDocument, position, queryGraphQLFn), {
            contents: {
                value: 'foo',
                kind: 'markdown',
            },
    ...
    it('should deal with empty payload', async () => {
        ...
        assert.deepStrictEqual(await hoverForPosition(textDocument, position, queryGraphQLFn), null)
    })
})
