...
describe('definitionForPosition', () => {
    it('should correctly parse result', async () => {
        ...
        assert.deepEqual(await definitionForPosition(textDocument, position, queryGraphQLFn), [
    ...
    it('should deal with empty payload', async () => {
        ...
        assert.deepEqual(await gatherValues(createProviders(queryGraphQLFn).definition(textDocument, position)), [null])
    })
})
