...
export function createProviderWrapper(languageSpec: LanguageSpec, logger: Logger): ProviderWrapper {
    ...
    return {
        definition: () => {
            const provider = createDefinitionProvider(lsifProviders.definition, searchProviders.definition)
            wrapped.definition = provider
            return provider
        },

        references: () => {
            const provider = createReferencesProvider(lsifProviders.references, searchProviders.references)
            wrapped.references = provider
            return provider
        },

        hover: () => {
            const provider = createHoverProvider(lsifProviders.hover, searchProviders.hover)
            wrapped.hover = provider
            return provider
        },

        documentHighlights: () => {
            const provider = createDocumentHighlightProvider(lsifProviders.documentHighlights)
